...
#     db=Db()
#     name=request.form['select']
#     qry=db.selectall("select * from complaint,login where complaint.login_id=login.login_id and complaint.complaint_id not in (select complaint_id from reply) and login.type='"+name+"'")
#     return jsonify()
...
def childdet():
   db=Db()
   name=request.form['name']
   qry=db.selectall("select * from child_reg where motheraadharno LIKE'%"+name+"%' and child_reg.child_reg_id not in (select hospital_child_reg_id from enroll_child)")
...
def userlogin():
    ...
    if res!=None:
        ...
        if type=='user':
            ...
            print(qr2)
            print(qr)
            return jsonify(status='ok',type=res[3],loginid=res[0],mksid=qr[0],angu_id=qryy[0],ad_no=qr1[0],mid=qr2[0],childid=qry4[0],child_id=qr[0])
        else:
...
def userviewnotification():
    ...
    print("hi")
    qry=db.jsonsel("SELECT notification_id,subject,massage,angu_id, DATE_FORMAT(date, '%d/%m/%Y') as date from notification where angu_id=43 and date>now()")
    print(qry)
    if len(qry)!=0:
        return jsonify(status='ok',data=qry)
    else:
...
def staffaddnutritin():
    ...
    qry="insert into vaccination_entry (vaccination_id,date,child_id,venuid,staff_id,type) values('"+vaccination_id+"',current_date ,'"+momid+"','"+angu_id+"','"+sid+"','child')"
    res=db.nonreturn(qry)
